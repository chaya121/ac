...
export default function FormPage({
  ...
  const handleCustomerChange = (val) => {
    setFormState(prev => ({
      ...prev,
      customer: val,
      brand: '' // Reset brand when customer changes
    }));
  };
  ...
  const getMachineBreakdown = (rows) => {
    ...
    rows.forEach(r => {
      const m = r.machine || '-';
      const workers = parseFloat(r.workers) || 1;
      map[m] = (map[m] || 0) + workers;
    });
    return map;
  };
  ...
  const availableBrands = useMemo(() => {
    if (formState.customer) {
      const customerBrands = [...new Set(records.filter(r => r.customer === formState.customer).map(r => r.brand).filter(Boolean))];
      const allBrands = masterLists.brands || [];
      const combined = [...new Set([...customerBrands, ...allBrands])];
      return sortThaiFirst(combined);
    }
    return sortedBrands;
  }, [records, formState.customer, masterLists.brands, sortedBrands]);
      ...
      <div className="form-card" style={{ marginBottom: '20px' }}>
        ...
        <div className="form-body">
              ...
              <select 
                ...
                onChange={(e) => {
                  if (e.target.value === '__custom__') {
                    const custom = prompt('พิมพ์ค่าที่ต้องการ:');
                    if (custom && custom.trim()) {
                      handleCustomerChange(custom.trim());
                    }
                  } else {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
            ...
            <div className="frow">
              <span className="flabel" style={{ minWidth: '130px', fontSize: '14px' }}>ชนิดเสื้อผ้า :</span>
              <select 
                className="finput"
                style={{ maxWidth: '200px', fontSize: '14px' }}
